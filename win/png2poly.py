...
while i < len(img):
	...
	im = Image.open(os.path.join(curDir,inFileKMZ,'files','%s.png' % fname))
	xsize = im.size[0]
	ysize = im.size[1]
...
for files in os.listdir(pngDir):
	if files.endswith(".png"):
		...
		print 'gdal_polygonize\gdal_polygonize.exe %s -f "ESRI Shapefile" %s %s' % (pngPath, outPath, fname)

		os.system('gdal_polygonize\gdal_polygonize.exe "%s" -f "ESRI Shapefile" "%s" %s' % (pngPath, outPath, fname))
		os.system('ogr2ogr.exe -f KML "%s" "%s"' % (kmlPath, os.path.join(outPath,'%s.shp' % fname)))
